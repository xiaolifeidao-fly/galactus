...
type DyJisuController struct {
	BaseController
}

func (c *DyJisuController) GetParams() {
	var v ReqStruct
	...
	err, HeaderInfo := JiSuIosMakeHeader(uint64(time_10), queryMd5[0:4], bodyMd5[0:4], query_hash_sm3[0:6], X_SS_Stub_Sm3[0:6], querySm3, ClientKeyMd5, v.DeviceId, v.ArgusDevicesToken, actionInfo, v.License, v.GorgonSdkVersionStr, v.Appid, v.AppVersion, v.ArgusSdkVersionStr, v.ArgusSdkVersion)
	if err != nil {
...
func JiSuIosMakeHeader(ts uint64, gorgon_querHash, gorgon_bodyHash, argus_queryHash, argus_bodyHash, argus_querySm3 []byte, clientKey, devices_id, device_token string, actionInfo *pb.XArgusV15, license, sdkVersionStr, appid string, argus_app_version, argus_sdk_version_str string, argus_sdk_version uint64) (err error, res *MyHeader) {
	var header = &MyHeader{}
	time_10_str := strconv.FormatInt(int64(ts), 10)
	header.XKhronos = time_10_str
	err, header.XArgus = JiSuIosGetXArgus(ts, argus_queryHash, argus_bodyHash, argus_querySm3, clientKey, devices_id, device_token, actionInfo, appid, license, argus_app_version, argus_sdk_version_str, argus_sdk_version)
	if err != nil {
		return err, nil
	}
	err, header.XGorgon = JiSuIosGetXGorgon(int64(ts), gorgon_querHash, gorgon_bodyHash, sdkVersionStr)
	if err != nil {
		return err, nil
	}
	err, header.XLadon = JiSuIosGetXLadon(int64(ts), appid, license)
	if err != nil {
		return err, nil
	}
	return nil, header
}
func JiSuIosGetXArgus(ts uint64, argus_queryHash, argus_bodyHash, argus_querySm3 []byte, clientKey, devices_id, device_token string, actionInfo *pb.XArgusV15, Appid, License, argus_app_version, argus_sdk_version_str string, argus_sdk_version uint64) (err error, res string) {
	...
	err, Xargus := ios.JiSuEncryptArgus(&p1)
	if err != nil {
		return err, ""
	}
	return nil, Xargus
}
func JiSuIosGetXGorgon(ts int64, gorgon_querHash, gorgon_bodyHash []byte, sdkVersionStr string) (err error, res string) {
...
func JiSuIosGetXLadon(ts int64, appid, license string) (err error, res string) {
...
func (c *DyJisuController) DecryptInfo() {
	...
	err, ArgusInfo := JiSuDecryptIosXargus(v.Argus)
	if err != nil {
		c.responseFail("解密[x-argus]失败!!", nil)
	}
	err, LadonInfo := JiSuDecryptIosXladon(v.Ladon)
	if err != nil {
	...
	err, GorgonInfo := JiSuDecryptIosGorgon(v.Gorgon)
	if err != nil {
...
func JiSuDecryptIosXargus(argus string) (err error, resInfo *pb.XArgus) {
...
func JiSuDecryptIosXladon(ladon string) (err error, resInfo string) {
	base64Str := ladon
...
func JiSuDecryptIosGorgon(gorgon string) (err error, resInfo *utils.XGorGonInfo) {
